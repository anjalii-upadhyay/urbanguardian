...
d3.csv(window.APP.CSV_PATH).then(rows => {
  ...
  fetch("/static/data/wards_with_pct.geojson")
    ...
    .then(data => {
      ...
      // Create UI controls
      createUIControls(colorScale);
      loadAQIData();
    }).catch(console.error);
});

function createUIControls(colorScale) {
  // Create toggles container in right panel
  const toggleContainer = document.createElement('div');
  toggleContainer.className = 'control-panel';
  ...
  `;
  document.querySelector('.right-panel').appendChild(toggleContainer);

  // Wire up the toggles
  document.getElementById('heat-toggle').addEventListener('change', e => {
    if (!heatLayer) return;
    heatLayer.setStyle({ fillOpacity: e.target.checked ? 0.4 : 0 });
  });
  ...
  legendContainer.innerHTML = `
    <h4>${window.APP.LAYER_NAMES.HEAT} (ΔT %)</h4>
    <div class="legend-item">
      <span class="legend-color" style="background: ${colorScale(0)};"></span>
      <span class="legend-label">Cool (low ΔT)</span>
    </div>
    <div class="legend-item">
      <span class="legend-color" style="background: ${colorScale(50)};"></span>
      <span class="legend-label">Medium ΔT</span>
    </div>
    <div class="legend-item">
      <span class="legend-color" style="background: ${colorScale(100)};"></span>
      <span class="legend-label">Hot (high ΔT)</span>
    </div>
  `;
  document.querySelector('.right-panel').appendChild(legendContainer);
}

function loadAQIData() {
  if (!wardsGeoJson) return;
